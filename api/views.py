...
def add_coach(request):
    ...
    # Check if required data is provided
    if not all([coach_id, first_name, last_name, email, gender,  phone, level,  username,room_id]):
        return Response({'error': 'All required fields must be provided.'}, status=400)
    ...
    try:
        ...
        with transaction.atomic():
            ...
            # Create the Coach User using the Profile
            coach_user = Coach.objects.create(user=coach_profile, coach_id=coach_id,room_id=room_id, first_name= first_name, last_name=last_name, email=email, phone=phone, level=level, education=education, rating=rating, 
