...
# Expected input_data "filters": [{"key":"area_of_expertise","value":"test"},...]
# @api_view(['POST'])
...
def format_data(input_data):
    competencies = [
        ...
        if "_" not in item and item != "Name" and item != "Email"
    ]
    res = []
    ...
    for user_data in input_data:
        user_dict = {
            ...
            "Self": [
                user_data[competency] if competency in user_data else ""
                for competency in competencies
            ],
            "Manager": [
                user_data["M_" + competency] if "M_" + competency in user_data else ""
                for competency in competencies
            ],
        }
...
def generate_individual_graphs(data, bar_width=0.35):
    competencies = data[0]["Competencies"]
    base64_images = {}

    for i, user_data in enumerate(data):
        user_name = user_data["name"]
        self_scores = user_data["Self"]
        manager_scores = user_data["Manager"]

        # Generate x-axis positions for each bar
        x_positions = np.arange(len(competencies))

        # Set up figure and axis with increased width to accommodate rotated labels
        ...
        ax.set_title(f"Competency Scores - {user_name}")
        ax.set_ylabel("Scores")
        ax.set_xticks(x_positions)
        ax.set_xticklabels(
            competencies, rotation=22, ha="right", fontsize="small"
        ...
        plt.savefig(f"graphsAndReports/{user_name}_graph.png", format="png")
        plt.close()

        # Convert the BytesIO object to base64
        img_base64 = base64.b64encode(img_buffer.getvalue()).decode("utf-8")
        base64_images[user_name] = img_base64
...
def format_data_for_html(data):
    ...
    for key, value in data.items():
        if key == "Name" or key.startswith("M_"):
            continue
        
        competency, _, question = key.rpartition('_')
        
        if competency not in formatted_data:
            formatted_data[competency] = []

        response_data = {
            'Question': question,
            'Self': value,
            'Manager': data.get(f'M_{competency}_{question}', None)
        }
        

        formatted_data[competency].append(response_data)
    final_data=[]
...
def freq_data_get(data):
    ...
    for chunk_data in data:
        ...
        temp_data = {
                "competency_name": chunk_data["competency_name"],
                "questions": []
            }
        
        
        for one_val in chunk_data["questions"]:
            
            temp_data2 = {
                    "Question": one_val["Question"],
                }
            
            val = {one_val["Self"]: 1}
            
            if one_val["Manager"] in val:
                val[one_val["Manager"]] = val[one_val["Manager"]] + 1
            else:
                val[one_val["Manager"]] = 1
            
            for l in label:
                if l not in val:
                    val[l] = ""

            temp_data2["values"] = val  # Convert values to list
            temp_data["questions"].append(temp_data2)
...
def transform_data(data):
    ...
    for competency, self_score, manager_score in zip(
        data["Competencies"], data["Self"], data["Manager"]
    ):
        transformed_data.append(
            {"Competency": competency, "Self": self_score, "Manager": manager_score}
        )
...
class download(APIView):
    def get(self, request):
        input_data = [
            {
                "Name": "Ajit G Khaparde",
                "Email": "akhaparde@medline.com",
                "Building Trust": 5,
                ...
                "Problem Solving": 5,
                "Giving Feedback": 5,
                "Entrepreneurial Mindset": 4.5,
                "M_Building Trust": 5,
                "M_Interpersonal Skills": 5,
                "M_Delegation Skill": 5,
                "M_Assertiveness": 4.75,
                "M_Influencing without Authority": 5,
                "M_Planning and Prioritizing": 4.67,
                "M_Problem Solving": 4.8,
                "M_Giving Feedback": 5,
                "M_Entrepreneurial Mindset": 5,
            },
            {
                "Name": "Akshay Sanjay Agrawal",
                "Email": "aagrawal@medline.com",
                "Building Trust": 3.75,
                "Interpersonal Skills": 3.5,
                "Delegation Skill": 3.33,
                "Assertiveness": 3.75,
                "Influencing without Authority": 4.5,
                "Planning and Prioritizing": 4.33,
                "Problem Solving": 4.2,
                "Giving Feedback": 3.33,
                "Entrepreneurial Mindset": 3.5,
                "M_Building Trust": 4.5,
                "M_Interpersonal Skills": 5,
                "M_Delegation Skill": 4.33,
                "M_Assertiveness": 5,
                "M_Influencing without Authority": 4.75,
                "M_Planning and Prioritizing": 4,
                "M_Problem Solving": 4.4,
                "M_Giving Feedback": 4.67,
                "M_Entrepreneurial Mindset": 4,
            },
            {
                "Name": "Basawaraj Biradar",
                "Email": "bbiradar@medline.com",
                "Building Trust": 3.75,
                "Interpersonal Skills": 4.75,
                "Delegation Skill": 4.67,
                "Assertiveness": 4.5,
                "Influencing without Authority": 5,
                "Planning and Prioritizing": 5,
                "Problem Solving": 5,
                "Giving Feedback": 4.33,
                "Entrepreneurial Mindset": 5,
                "M_Building Trust": 4.75,
                "M_Interpersonal Skills": 4.5,
                "M_Delegation Skill": 3.67,
                "M_Assertiveness": 4,
                "M_Influencing without Authority": 4.5,
                "M_Planning and Prioritizing": 4.33,
                "M_Problem Solving": 5,
                "M_Giving Feedback": 4.67,
                "M_Entrepreneurial Mindset": 3,
            },
            {
                "Name": "Deepak Mishra",
                "Email": "dsmishra@medline.com",
                "Building Trust": 3.75,
                "Interpersonal Skills": 4.25,
                "Delegation Skill": 4,
                "Assertiveness": 3.75,
                "Influencing without Authority": 4,
                "Planning and Prioritizing": 3.67,
                "Problem Solving": 4,
                "Giving Feedback": 4.33,
                "Entrepreneurial Mindset": 4,
                "M_Building Trust": 4,
                "M_Interpersonal Skills": 4,
                "M_Delegation Skill": 3,
                "M_Assertiveness": 3.75,
                "M_Influencing without Authority": 4,
                "M_Planning and Prioritizing": 3,
                "M_Problem Solving": 3.6,
                "M_Giving Feedback": 3.67,
                "M_Entrepreneurial Mindset": 3,
            },
            {
                "Name": "Harshad Ramesh Suryawaunshi",
                "Email": "hsuryawaunshi@medline.com",
                "Building Trust": 3.75,
                "Interpersonal Skills": 4.25,
                "Delegation Skill": 4,
                "Assertiveness": 3.25,
                ...
                "Giving Feedback": 4,
                "Entrepreneurial Mindset": 4,
                "M_Building Trust": 3.5,
                "M_Interpersonal Skills": 3.5,
                "M_Delegation Skill": 3.67,
                "M_Assertiveness": 2.5,
                "M_Influencing without Authority": 3.75,
                "M_Planning and Prioritizing": 3,
                "M_Problem Solving": 3.6,
                "M_Giving Feedback": 3.67,
                "M_Entrepreneurial Mindset": 3,
            ...
            {
                "Name": "K S Vaibhav",
                "Email": "vkandala@medline.com",
                "Building Trust": 4,
                "Interpersonal Skills": 4,
                "Delegation Skill": 3.67,
                "Assertiveness": 3.5,
                "Influencing without Authority": 4,
                "Planning and Prioritizing": 4,
                "Problem Solving": 3.8,
                "Giving Feedback": 3.67,
                "Entrepreneurial Mindset": 2.5,
                "M_Building Trust": 5,
                "M_Interpersonal Skills": 4.5,
                "M_Delegation Skill":4.33,
                "M_Assertiveness": 4.25,
                "M_Influencing without Authority": 4.5,
                "M_Planning and Prioritizing": 4.33,
                "M_Problem Solving": 4.8,
                "M_Giving Feedback": 4.67,
            ...
            {
                "Name": "Pradeep Zende",
                "Email": "pzende@medline.com",
                "Building Trust": 4.75,
                "Interpersonal Skills": 4.25,
                "Delegation Skill": 4.33,
                "Assertiveness": 4,
                "Influencing without Authority": 4.5,
                "Planning and Prioritizing": 4.67,
                "Problem Solving": 4.2,
                "Giving Feedback": 4,
                "Entrepreneurial Mindset": 3.5,
                "M_Building Trust": 3.5,
                "M_Interpersonal Skills": 2.75,
                "M_Delegation Skill": 3.67,
                "M_Assertiveness": 3.25,
                "M_Influencing without Authority": 3.25,
                "M_Planning and Prioritizing": 4,
                "M_Problem Solving": 3.6,
                "M_Giving Feedback": 2.67,
                "M_Entrepreneurial Mindset": 3,
            },
            {
                "Name": "Pritam Hegishte",
                "Email": "phegishte@medline.com",
                "Building Trust": 5,
                "Interpersonal Skills": 5,
                "Delegation Skill": 5,
                "Assertiveness": 4.75,
                "Influencing without Authority": 4.25,
                "Planning and Prioritizing": 5,
                "Problem Solving": 5,
                "Giving Feedback": 5,
                "Entrepreneurial Mindset": 5,
                "M_Building Trust": 3.5,
                "M_Interpersonal Skills": 3.5,
                "M_Delegation Skill": 3.33,
                "M_Assertiveness": 3.75,
                "M_Influencing without Authority": 3.25,
                "M_Planning and Prioritizing": 2.67,
                "M_Problem Solving": 3.8,
                "M_Giving Feedback": 2.67,
                "M_Entrepreneurial Mindset": 3,
            },
            {
                "Name": "Priyadarshani Priyanka",
                "Email": "ppriyadarshani@medline.com",
                "Building Trust": 5,
                ...
                "Planning and Prioritizing": 5,
                "Problem Solving": 4.4,
                "Giving Feedback": 4.67,
                "Entrepreneurial Mindset": 5,
                "M_Building Trust": 4.63,
                "M_Interpersonal Skills": 4.88,
                "M_Delegation Skill": 4.17,
                "M_Assertiveness": 4.25,
                "M_Influencing without Authority": 4.38,
                "M_Planning and Prioritizing": 4.17,
                "M_Problem Solving": 4,
                "M_Giving Feedback": 4.5,
                "M_Entrepreneurial Mindset": 4,
            },
            {
                "Name": "Rajendra Kumar Rana",
                "Email": "rrana@medline.com",
                "Building Trust": 4.75,
                "Interpersonal Skills": 4.25,
                "Delegation Skill": 5,
                "Assertiveness": 4.5,
            ...
            {
                "Name": "Rakesh Kumar",
                "Email": "rakumar@medline.com",
                "Building Trust": 4.75,
                "Interpersonal Skills": 4.75,
                "Delegation Skill": 3.67,
                "Assertiveness": 3.5,
                "Influencing without Authority": 4.75,
                ...
                "M_Assertiveness": 4,
                "M_Influencing without Authority": 4.75,
                "M_Planning and Prioritizing": 3.67,
                "M_Problem Solving": 4.4,
                "M_Giving Feedback": 3.67,
                "M_Entrepreneurial Mindset": 3.5,
            },
            {
                "Name": "Ravish Kumar Singh",
                "Email": "rssingh@medline.com",
                "Building Trust": 4.25,
                "Interpersonal Skills": 3.25,
                "Delegation Skill": 3.33,
                "Assertiveness": 4.75,
                "Influencing without Authority": 4.5,
                "Planning and Prioritizing": 4.33,
                ...
                "M_Delegation Skill": 3.67,
                "M_Assertiveness": 2.5,
                "M_Influencing without Authority": 2.5,
                "M_Planning and Prioritizing": 3.33,
                "M_Problem Solving": 3.6,
                "M_Giving Feedback": 3.33,
                "M_Entrepreneurial Mindset": 2,
            },
            {
                "Name": "Sagar Satish Nawathe",
                "Email": "snawathe@medline.com",
                "Building Trust": 5,
                "Interpersonal Skills": 4.5,
                "Delegation Skill": 5,
                "Assertiveness": 5,
            ...
            {
                "Name": "Sooraj Pathania",
                "Email": "spathania@medline.com",
                "Building Trust": 4.5,
                "Interpersonal Skills": 4.5,
                "Delegation Skill": 4.67,
                "Assertiveness": 4,
                ...
                "Entrepreneurial Mindset": 4.5,
                "M_Building Trust": 3.5,
                "M_Interpersonal Skills": 1.25,
                "M_Delegation Skill": 3,
                "M_Assertiveness": 3.5,
                "M_Influencing without Authority": 3.25,
                "M_Planning and Prioritizing": 3,
                "M_Problem Solving": 2.6,
                "M_Giving Feedback": 2.67,
                "M_Entrepreneurial Mindset": 2,
            },
            {
                "Name": "Sushil Shivajirao Deshpande",
                "Email": "sdeshpande@medline.com",
                "Building Trust": 4.25,
                "Interpersonal Skills": 4.75,
                "Delegation Skill": 4.67,
                "Assertiveness": 4,
                ...
                "Giving Feedback": 4.33,
                "Entrepreneurial Mindset": 4,
                "M_Building Trust": 4.25,
                "M_Interpersonal Skills": 4.5,
                "M_Delegation Skill": 4,
                "M_Assertiveness": 4.5,
                "M_Influencing without Authority": 4.25,
                "M_Planning and Prioritizing": 4,
                "M_Problem Solving": 4.4,
                "M_Giving Feedback": 5,
                "M_Entrepreneurial Mindset": 3.5,
            ...
            {
                "Name": "Vibhas Kelkar",
                "Email": "vkelkar@medline.com",
                "Building Trust": 4.5,
                "Interpersonal Skills": 4.5,
                "Delegation Skill": 4.33,
                "Assertiveness": 3.25,
                "Influencing without Authority": 3.75,
                "Planning and Prioritizing": 4.67,
                "Problem Solving": 4.2,
                "Giving Feedback": 4.33,
                "Entrepreneurial Mindset": 3,
                "M_Building Trust": 4.25,
                "M_Interpersonal Skills": 3.75,
                "M_Delegation Skill": 3.67,
                "M_Assertiveness": 3.75,
                "M_Influencing without Authority": 4,
                "M_Planning and Prioritizing": 4,
                "M_Problem Solving": 3.8,
                "M_Giving Feedback": 4.33,
                "M_Entrepreneurial Mindset": 3,
            },
            {
                "Name": "Vinod Jibhau Ahire",
                "Email": "vahire@medline.com",
                "Building Trust": 4.25,
                "Interpersonal Skills": 4,
                "Delegation Skill": 3.67,
                "Assertiveness": 4.75,
        ...
        score_analysis_datas =    [
    {
      "Name": "Ajit G Khaparde",
      "Building Trust_I understand my team's needs and base my actions to support those needs.": 5,
      "Building Trust_My team members and I are approachable, openly communicate with each other and can share feedback without any fears.": 5,
      "Building Trust_I am always reliable / dependable": 5,
      "Building Trust_I treat everyone fairly and equally.": 5,
      "Interpersonal Skills_I make others feel positive and respected  by my words and gestures.": 5,
      "Interpersonal Skills_I take initiative to connect with people.": 5,
      "Interpersonal Skills_I display enthusiasm while interacting with others.": 5,
      "Interpersonal Skills_I connect with people outside my team and often act as a liason between teams and stakeholder.": 5,
      "Delegation Skills_I effectively delegate tasks to team members based on their skills and expertise.": 5,
      "Delegation Skills_I provide clear instructions and set expectations when assigning tasks to team members": 5,
      "Delegation Skills_I monitor delegated tasks while allowing team members to approach their assigned responsibilities in their own way.": 5,
      "Assertiveness_I am confident and clear while interacting with peers, reports, managers and stakeholders.": 5,
      "Assertiveness_I am sure of my perspectives - issues, solutions, way forward etc., and state them clearly to others.": 5,
      "Assertiveness_I can challenge the views of others when required.": 5,
      "Assertiveness_I come across as assertive when I interact with my team, managers and higherups.": 5,
      "Influencing without Authority_I use logic, facts and data to influence decisions and actions among peers and team members.": 5,
      "Influencing without Authority_I am open to listen to different views and opinions.": 5,
      "Influencing without Authority_When my thoughts and ideas are challenged, I am open and willing to provide clear explanations and rationales.": 5,
      "Influencing without Authority_I look for common ground and areas of agreement among peers or team members when making decisions.": 5,
      "Planning and Prioritizing_I plan my work effectively to meet timelines and quality.": 5,
      "Planning and Prioritizing_I prioritise my tasks and goals when there are multiple tasks at hand.": 5,
      "Planning and Prioritizing_I guide my team to plan and prioritise  work deliverables and meet timelines wihtout compromising on quality.": 5,
      "Problem Solving_I try to identify root causes while solving problems instead of finding quick fixes": 5,
      "Problem Solving_I use mine and the team's knowledge, reasoning and technical skills to evaluate key issues of a problem.": 5,
      "Problem Solving_I use best practices, gain new insights from problems  and processes to find innovative solutions.": 5,
      "Problem Solving_I look for ways to continuously improve things and learn from experiences.": 5,
      "Problem Solving_I use previous experiences to solve problems": 5,
      "Giving Feedback_I often talk to team members and guide them on their deliverables": 5,
      "Giving Feedback_I regularly tell team members what I think about their work and how they are doing and invite them to talk openly about their thoughts on the suggestions given.": 5,
      "Giving Feedback_I believe in open conversations and am open to receiving feedback from peers and team members": 5,
      "Entrepreneurial Mindset_I constantly look for new problems that need solutions - these can be at work or at a global level.": 5,
      "Entrepreneurial Mindset_I see the potential that problems bring with them.": 4,
      "M_Building Trust_I understand my team's needs and base my actions to support those needs.": 5,
      "M_Building Trust_My team members and I are approachable, openly communicate with each other and can share feedback without any fears.": 5,
      "M_Building Trust_I am always reliable / dependable": 5,
      "M_Building Trust_I treat everyone fairly and equally.": 5,
      "M_Interpersonal Skills_I make others feel positive and respected  by my words and gestures.": 5,
      "M_Interpersonal Skills_I take initiative to connect with people.": 5,
      "M_Interpersonal Skills_I display enthusiasm while interacting with others.": 5,
      "M_Interpersonal Skills_I connect with people outside my team and often act as a liason between teams and stakeholder.": 5,
      "M_Delegation Skills_I effectively delegate tasks to team members based on their skills and expertise.": 5,
      "M_Delegation Skills_I provide clear instructions and set expectations when assigning tasks to team members": 5,
      "M_Delegation Skills_I monitor delegated tasks while allowing team members to approach their assigned responsibilities in their own way.": 5,
      "M_Assertiveness_I am confident and clear while interacting with peers, reports, managers and stakeholders.": 5,
      "M_Assertiveness_I am sure of my perspectives - issues, solutions, way forward etc., and state them clearly to others.": 5,
      "M_Assertiveness_I can challenge the views of others when required.": 4,
      "M_Assertiveness_I come across as assertive when I interact with my team, managers and higherups.": 5,
      "M_Influencing without Authority_I use logic, facts and data to influence decisions and actions among peers and team members.": 5,
      "M_Influencing without Authority_I am open to listen to different views and opinions.": 5,
      "M_Influencing without Authority_When my thoughts and ideas are challenged, I am open and willing to provide clear explanations and rationales.": 5,
      "M_Influencing without Authority_I look for common ground and areas of agreement among peers or team members when making decisions.": 5,
      "M_Planning and Prioritizing_I plan my work effectively to meet timelines and quality.": 4,
      "M_Planning and Prioritizing_I prioritise my tasks and goals when there are multiple tasks at hand.": 5,
      "M_Planning and Prioritizing_I guide my team to plan and prioritise  work deliverables and meet timelines wihtout compromising on quality.": 5,
      "M_Problem Solving_I try to identify root causes while solving problems instead of finding quick fixes": 5,
      "M_Problem Solving_I use mine and the team's knowledge, reasoning and technical skills to evaluate key issues of a problem.": 5,
      "M_Problem Solving_I use best practices, gain new insights from problems  and processes to find innovative solutions.": 5,
      "M_Problem Solving_I look for ways to continuously improve things and learn from experiences.": 4,
      "M_Problem Solving_I use previous experiences to solve problems": 5,
      "M_Giving Feedback_I often talk to team members and guide them on their deliverables": 5,
      "M_Giving Feedback_I regularly tell team members what I think about their work and how they are doing and invite them to talk openly about their thoughts on the suggestions given.": 5,
      "M_Giving Feedback_I believe in open conversations and am open to receiving feedback from peers and team members": 5,
      "M_Entrepreneurial Mindset_I constantly look for new problems that need solutions - these can be at work or at a global level.": 5,
      "M_Entrepreneurial Mindset_I see the potential that problems bring with them.": 5
    },
    {
      "Name": "Akshay Sanjay Agrawal",
      "Building Trust_I understand my team's needs and base my actions to support those needs.": 4,
      "Building Trust_My team members and I are approachable, openly communicate with each other and can share feedback without any fears.": 3,
      "Building Trust_I am always reliable / dependable": 4,
      "Building Trust_I treat everyone fairly and equally.": 4,
      "Interpersonal Skills_I make others feel positive and respected  by my words and gestures.": 4,
      "Interpersonal Skills_I take initiative to connect with people.": 3,
      "Interpersonal Skills_I display enthusiasm while interacting with others.": 3,
      "Interpersonal Skills_I connect with people outside my team and often act as a liason between teams and stakeholder.": 4,
      "Delegation Skills_I effectively delegate tasks to team members based on their skills and expertise.": 3,
      "Delegation Skills_I provide clear instructions and set expectations when assigning tasks to team members": 4,
      "Delegation Skills_I monitor delegated tasks while allowing team members to approach their assigned responsibilities in their own way.": 3,
      "Assertiveness_I am confident and clear while interacting with peers, reports, managers and stakeholders.": 4,
      "Assertiveness_I am sure of my perspectives - issues, solutions, way forward etc., and state them clearly to others.": 4,
      "Assertiveness_I can challenge the views of others when required.": 4,
      "Assertiveness_I come across as assertive when I interact with my team, managers and higherups.": 3,
      "Influencing without Authority_I use logic, facts and data to influence decisions and actions among peers and team members.": 5,
      "Influencing without Authority_I am open to listen to different views and opinions.": 5,
      "Influencing without Authority_When my thoughts and ideas are challenged, I am open and willing to provide clear explanations and rationales.": 4,
      "Influencing without Authority_I look for common ground and areas of agreement among peers or team members when making decisions.": 4,
      "Planning and Prioritizing_I plan my work effectively to meet timelines and quality.": 5,
      "Planning and Prioritizing_I prioritise my tasks and goals when there are multiple tasks at hand.": 4,
      "Planning and Prioritizing_I guide my team to plan and prioritise  work deliverables and meet timelines wihtout compromising on quality.": 4,
      "Problem Solving_I try to identify root causes while solving problems instead of finding quick fixes": 5,
      "Problem Solving_I use mine and the team's knowledge, reasoning and technical skills to evaluate key issues of a problem.": 4,
      "Problem Solving_I use best practices, gain new insights from problems  and processes to find innovative solutions.": 4,
      "Problem Solving_I look for ways to continuously improve things and learn from experiences.": 5,
      "Problem Solving_I use previous experiences to solve problems": 3,
      "Giving Feedback_I often talk to team members and guide them on their deliverables": 4,
      "Giving Feedback_I regularly tell team members what I think about their work and how they are doing and invite them to talk openly about their thoughts on the suggestions given.": 2,
      "Giving Feedback_I believe in open conversations and am open to receiving feedback from peers and team members": 4,
      "Entrepreneurial Mindset_I constantly look for new problems that need solutions - these can be at work or at a global level.": 4,
      "Entrepreneurial Mindset_I see the potential that problems bring with them.": 3,
      "M_Building Trust_I understand my team's needs and base my actions to support those needs.": 4,
      "M_Building Trust_My team members and I are approachable, openly communicate with each other and can share feedback without any fears.": 4,
      "M_Building Trust_I am always reliable / dependable": 5,
      "M_Building Trust_I treat everyone fairly and equally.": 5,
      "M_Interpersonal Skills_I make others feel positive and respected  by my words and gestures.": 5,
      "M_Interpersonal Skills_I take initiative to connect with people.": 5,
      "M_Interpersonal Skills_I display enthusiasm while interacting with others.": 5,
      "M_Interpersonal Skills_I connect with people outside my team and often act as a liason between teams and stakeholder.": 5,
      "M_Delegation Skills_I effectively delegate tasks to team members based on their skills and expertise.": 4,
      "M_Delegation Skills_I provide clear instructions and set expectations when assigning tasks to team members": 5,
      "M_Delegation Skills_I monitor delegated tasks while allowing team members to approach their assigned responsibilities in their own way.": 4,
      "M_Assertiveness_I am confident and clear while interacting with peers, reports, managers and stakeholders.": 5,
      "M_Assertiveness_I am sure of my perspectives - issues, solutions, way forward etc., and state them clearly to others.": 5,
      "M_Assertiveness_I can challenge the views of others when required.": 5,
      "M_Assertiveness_I come across as assertive when I interact with my team, managers and higherups.": 5,
      "M_Influencing without Authority_I use logic, facts and data to influence decisions and actions among peers and team members.": 5,
      "M_Influencing without Authority_I am open to listen to different views and opinions.": 4,
      "M_Influencing without Authority_When my thoughts and ideas are challenged, I am open and willing to provide clear explanations and rationales.": 5,
      "M_Influencing without Authority_I look for common ground and areas of agreement among peers or team members when making decisions.": 5,
      "M_Planning and Prioritizing_I plan my work effectively to meet timelines and quality.": 4,
      "M_Planning and Prioritizing_I prioritise my tasks and goals when there are multiple tasks at hand.": 4,
      "M_Planning and Prioritizing_I guide my team to plan and prioritise  work deliverables and meet timelines wihtout compromising on quality.": 4,
      "M_Problem Solving_I try to identify root causes while solving problems instead of finding quick fixes": 5,
      "M_Problem Solving_I use mine and the team's knowledge, reasoning and technical skills to evaluate key issues of a problem.": 5,
      "M_Problem Solving_I use best practices, gain new insights from problems  and processes to find innovative solutions.": 4,
      "M_Problem Solving_I look for ways to continuously improve things and learn from experiences.": 4,
      "M_Problem Solving_I use previous experiences to solve problems": 4,
      "M_Giving Feedback_I often talk to team members and guide them on their deliverables": 5,
      "M_Giving Feedback_I regularly tell team members what I think about their work and how they are doing and invite them to talk openly about their thoughts on the suggestions given.": 4,
      "M_Giving Feedback_I believe in open conversations and am open to receiving feedback from peers and team members": 5,
      "M_Entrepreneurial Mindset_I constantly look for new problems that need solutions - these can be at work or at a global level.": 4,
      "M_Entrepreneurial Mindset_I see the potential that problems bring with them.": 4
    },
    {
      "Name": "Basawaraj Biradar",
      "Building Trust_I understand my team's needs and base my actions to support those needs.": 5,
      "Building Trust_My team members and I are approachable, openly communicate with each other and can share feedback without any fears.": 5,
      "Building Trust_I am always reliable / dependable": 0,
      "Building Trust_I treat everyone fairly and equally.": 5,
      "Interpersonal Skills_I make others feel positive and respected  by my words and gestures.": 5,
      "Interpersonal Skills_I take initiative to connect with people.": 5,
      "Interpersonal Skills_I display enthusiasm while interacting with others.": 5,
      ...
      "Delegation Skills_I effectively delegate tasks to team members based on their skills and expertise.": 5,
      "Delegation Skills_I provide clear instructions and set expectations when assigning tasks to team members": 5,
      "Delegation Skills_I monitor delegated tasks while allowing team members to approach their assigned responsibilities in their own way.": 4,
      "Assertiveness_I am confident and clear while interacting with peers, reports, managers and stakeholders.": 5,
      "Assertiveness_I am sure of my perspectives - issues, solutions, way forward etc., and state them clearly to others.": 5,
      "Assertiveness_I can challenge the views of others when required.": 4,
      "Assertiveness_I come across as assertive when I interact with my team, managers and higherups.": 4,
      "Influencing without Authority_I use logic, facts and data to influence decisions and actions among peers and team members.": 5,
      "Influencing without Authority_I am open to listen to different views and opinions.": 5,
      "Influencing without Authority_When my thoughts and ideas are challenged, I am open and willing to provide clear explanations and rationales.": 5,
      "Influencing without Authority_I look for common ground and areas of agreement among peers or team members when making decisions.": 5,
      ...
      "Problem Solving_I use previous experiences to solve problems": 5,
      "Giving Feedback_I often talk to team members and guide them on their deliverables": 5,
      "Giving Feedback_I regularly tell team members what I think about their work and how they are doing and invite them to talk openly about their thoughts on the suggestions given.": 4,
      "Giving Feedback_I believe in open conversations and am open to receiving feedback from peers and team members": 4,
      "Entrepreneurial Mindset_I constantly look for new problems that need solutions - these can be at work or at a global level.": 5,
      "Entrepreneurial Mindset_I see the potential that problems bring with them.": 5,
      "M_Building Trust_I understand my team's needs and base my actions to support those needs.": 4,
      "M_Building Trust_My team members and I are approachable, openly communicate with each other and can share feedback without any fears.": 5,
      "M_Building Trust_I am always reliable / dependable": 5,
      "M_Building Trust_I treat everyone fairly and equally.": 5,
      "M_Interpersonal Skills_I make others feel positive and respected  by my words and gestures.": 4,
      "M_Interpersonal Skills_I take initiative to connect with people.": 5,
      "M_Interpersonal Skills_I display enthusiasm while interacting with others.": 5,
      "M_Interpersonal Skills_I connect with people outside my team and often act as a liason between teams and stakeholder.": 4,
      "M_Delegation Skills_I effectively delegate tasks to team members based on their skills and expertise.": 3,
      "M_Delegation Skills_I provide clear instructions and set expectations when assigning tasks to team members": 4,
      "M_Delegation Skills_I monitor delegated tasks while allowing team members to approach their assigned responsibilities in their own way.": 4,
      "M_Assertiveness_I am confident and clear while interacting with peers, reports, managers and stakeholders.": 4,
      "M_Assertiveness_I am sure of my perspectives - issues, solutions, way forward etc., and state them clearly to others.": 4,
      "M_Assertiveness_I can challenge the views of others when required.": 5,
      "M_Assertiveness_I come across as assertive when I interact with my team, managers and higherups.": 3,
      "M_Influencing without Authority_I use logic, facts and data to influence decisions and actions among peers and team members.": 4,
      "M_Influencing without Authority_I am open to listen to different views and opinions.": 4,
      "M_Influencing without Authority_When my thoughts and ideas are challenged, I am open and willing to provide clear explanations and rationales.": 5,
      "M_Influencing without Authority_I look for common ground and areas of agreement among peers or team members when making decisions.": 5,
      "M_Planning and Prioritizing_I plan my work effectively to meet timelines and quality.": 4,
      ...
      "M_Problem Solving_I use mine and the team's knowledge, reasoning and technical skills to evaluate key issues of a problem.": 5,
      "M_Problem Solving_I use best practices, gain new insights from problems  and processes to find innovative solutions.": 5,
      "M_Problem Solving_I look for ways to continuously improve things and learn from experiences.": 5,
      "M_Problem Solving_I use previous experiences to solve problems": 5,
      "M_Giving Feedback_I often talk to team members and guide them on their deliverables": 5,
      "M_Giving Feedback_I regularly tell team members what I think about their work and how they are doing and invite them to talk openly about their thoughts on the suggestions given.": 4,
      "M_Giving Feedback_I believe in open conversations and am open to receiving feedback from peers and team members": 5,
      "M_Entrepreneurial Mindset_I constantly look for new problems that need solutions - these can be at work or at a global level.": 3,
      "M_Entrepreneurial Mindset_I see the potential that problems bring with them.": 3
    },
    {
      "Name": "Deepak Mishra",
      "Building Trust_I understand my team's needs and base my actions to support those needs.": 3,
      "Building Trust_My team members and I are approachable, openly communicate with each other and can share feedback without any fears.": 4,
      "Building Trust_I am always reliable / dependable": 5,
      "Building Trust_I treat everyone fairly and equally.": 3,
      "Interpersonal Skills_I make others feel positive and respected  by my words and gestures.": 4,
      "Interpersonal Skills_I take initiative to connect with people.": 4,
      "Interpersonal Skills_I display enthusiasm while interacting with others.": 4,
      "Interpersonal Skills_I connect with people outside my team and often act as a liason between teams and stakeholder.": 5,
      "Delegation Skills_I effectively delegate tasks to team members based on their skills and expertise.": 4,
      "Delegation Skills_I provide clear instructions and set expectations when assigning tasks to team members": 4,
      "Delegation Skills_I monitor delegated tasks while allowing team members to approach their assigned responsibilities in their own way.": 4,
      "Assertiveness_I am confident and clear while interacting with peers, reports, managers and stakeholders.": 4,
      "Assertiveness_I am sure of my perspectives - issues, solutions, way forward etc., and state them clearly to others.": 4,
      "Assertiveness_I can challenge the views of others when required.": 3,
      "Assertiveness_I come across as assertive when I interact with my team, managers and higherups.": 4,
      "Influencing without Authority_I use logic, facts and data to influence decisions and actions among peers and team members.": 4,
      "Influencing without Authority_I am open to listen to different views and opinions.": 4,
      "Influencing without Authority_When my thoughts and ideas are challenged, I am open and willing to provide clear explanations and rationales.": 4,
      "Influencing without Authority_I look for common ground and areas of agreement among peers or team members when making decisions.": 4,
      "Planning and Prioritizing_I plan my work effectively to meet timelines and quality.": 4,
      "Planning and Prioritizing_I prioritise my tasks and goals when there are multiple tasks at hand.": 4,
      "Planning and Prioritizing_I guide my team to plan and prioritise  work deliverables and meet timelines wihtout compromising on quality.": 3,
      "Problem Solving_I try to identify root causes while solving problems instead of finding quick fixes": 4,
      "Problem Solving_I use mine and the team's knowledge, reasoning and technical skills to evaluate key issues of a problem.": 4,
      "Problem Solving_I use best practices, gain new insights from problems  and processes to find innovative solutions.": 4,
      "Problem Solving_I look for ways to continuously improve things and learn from experiences.": 4,
      "Problem Solving_I use previous experiences to solve problems": 4,
      "Giving Feedback_I often talk to team members and guide them on their deliverables": 4,
      "Giving Feedback_I regularly tell team members what I think about their work and how they are doing and invite them to talk openly about their thoughts on the suggestions given.": 4,
      "Giving Feedback_I believe in open conversations and am open to receiving feedback from peers and team members": 5,
      "Entrepreneurial Mindset_I constantly look for new problems that need solutions - these can be at work or at a global level.": 4,
      ...
      "M_Building Trust_I understand my team's needs and base my actions to support those needs.": 4,
      "M_Building Trust_My team members and I are approachable, openly communicate with each other and can share feedback without any fears.": 4,
      "M_Building Trust_I am always reliable / dependable": 4,
      "M_Building Trust_I treat everyone fairly and equally.": 4,
      "M_Interpersonal Skills_I make others feel positive and respected  by my words and gestures.": 4,
      "M_Interpersonal Skills_I take initiative to connect with people.": 4,
      "M_Interpersonal Skills_I display enthusiasm while interacting with others.": 4,
      "M_Interpersonal Skills_I connect with people outside my team and often act as a liason between teams and stakeholder.": 4,
      "M_Delegation Skills_I effectively delegate tasks to team members based on their skills and expertise.": 3,
      "M_Delegation Skills_I provide clear instructions and set expectations when assigning tasks to team members": 3,
      "M_Delegation Skills_I monitor delegated tasks while allowing team members to approach their assigned responsibilities in their own way.": 3,
      "M_Assertiveness_I am confident and clear while interacting with peers, reports, managers and stakeholders.": 4,
      "M_Assertiveness_I am sure of my perspectives - issues, solutions, way forward etc., and state them clearly to others.": 3,
      "M_Assertiveness_I can challenge the views of others when required.": 4,
      "M_Assertiveness_I come across as assertive when I interact with my team, managers and higherups.": 4,
      "M_Influencing without Authority_I use logic, facts and data to influence decisions and actions among peers and team members.": 4,
      "M_Influencing without Authority_I am open to listen to different views and opinions.": 4,
      "M_Influencing without Authority_When my thoughts and ideas are challenged, I am open and willing to provide clear explanations and rationales.": 4,
      "M_Influencing without Authority_I look for common ground and areas of agreement among peers or team members when making decisions.": 4,
      "M_Planning and Prioritizing_I plan my work effectively to meet timelines and quality.": 3,
      "M_Planning and Prioritizing_I prioritise my tasks and goals when there are multiple tasks at hand.": 3,
      "M_Planning and Prioritizing_I guide my team to plan and prioritise  work deliverables and meet timelines wihtout compromising on quality.": 3,
      "M_Problem Solving_I try to identify root causes while solving problems instead of finding quick fixes": 4,
      "M_Problem Solving_I use mine and the team's knowledge, reasoning and technical skills to evaluate key issues of a problem.": 4,
      "M_Problem Solving_I use best practices, gain new insights from problems  and processes to find innovative solutions.": 3,
      "M_Problem Solving_I look for ways to continuously improve things and learn from experiences.": 3,
      "M_Problem Solving_I use previous experiences to solve problems": 4,
      "M_Giving Feedback_I often talk to team members and guide them on their deliverables": 4,
      "M_Giving Feedback_I regularly tell team members what I think about their work and how they are doing and invite them to talk openly about their thoughts on the suggestions given.": 3,
      "M_Giving Feedback_I believe in open conversations and am open to receiving feedback from peers and team members": 4,
      "M_Entrepreneurial Mindset_I constantly look for new problems that need solutions - these can be at work or at a global level.": 3,
      "M_Entrepreneurial Mindset_I see the potential that problems bring with them.": 3
    },
    {
      ...
      "Delegation Skills_I provide clear instructions and set expectations when assigning tasks to team members": 4,
      "Delegation Skills_I monitor delegated tasks while allowing team members to approach their assigned responsibilities in their own way.": 4,
      "Assertiveness_I am confident and clear while interacting with peers, reports, managers and stakeholders.": 3,
      "Assertiveness_I am sure of my perspectives - issues, solutions, way forward etc., and state them clearly to others.": 3,
      "Assertiveness_I can challenge the views of others when required.": 3,
      "Assertiveness_I come across as assertive when I interact with my team, managers and higherups.": 4,
      "Influencing without Authority_I use logic, facts and data to influence decisions and actions among peers and team members.": 4,
      "Influencing without Authority_I am open to listen to different views and opinions.": 4,
      ...
      "Problem Solving_I use previous experiences to solve problems": 4,
      "Giving Feedback_I often talk to team members and guide them on their deliverables": 4,
      "Giving Feedback_I regularly tell team members what I think about their work and how they are doing and invite them to talk openly about their thoughts on the suggestions given.": 3,
      "Giving Feedback_I believe in open conversations and am open to receiving feedback from peers and team members": 5,
      "Entrepreneurial Mindset_I constantly look for new problems that need solutions - these can be at work or at a global level.": 4,
      "Entrepreneurial Mindset_I see the potential that problems bring with them.": 4,
      "M_Building Trust_I understand my team's needs and base my actions to support those needs.": 3,
      "M_Building Trust_My team members and I are approachable, openly communicate with each other and can share feedback without any fears.": 3,
      "M_Building Trust_I am always reliable / dependable": 4,
      "M_Building Trust_I treat everyone fairly and equally.": 4,
      "M_Interpersonal Skills_I make others feel positive and respected  by my words and gestures.": 4,
      "M_Interpersonal Skills_I take initiative to connect with people.": 3,
      "M_Interpersonal Skills_I display enthusiasm while interacting with others.": 3,
      "M_Interpersonal Skills_I connect with people outside my team and often act as a liason between teams and stakeholder.": 4,
      "M_Delegation Skills_I effectively delegate tasks to team members based on their skills and expertise.": 4,
      "M_Delegation Skills_I provide clear instructions and set expectations when assigning tasks to team members": 3,
      "M_Delegation Skills_I monitor delegated tasks while allowing team members to approach their assigned responsibilities in their own way.": 4,
      "M_Assertiveness_I am confident and clear while interacting with peers, reports, managers and stakeholders.": 3,
      "M_Assertiveness_I am sure of my perspectives - issues, solutions, way forward etc., and state them clearly to others.": 3,
      "M_Assertiveness_I can challenge the views of others when required.": 2,
      "M_Assertiveness_I come across as assertive when I interact with my team, managers and higherups.": 2,
      "M_Influencing without Authority_I use logic, facts and data to influence decisions and actions among peers and team members.": 3,
      "M_Influencing without Authority_I am open to listen to different views and opinions.": 4,
      "M_Influencing without Authority_When my thoughts and ideas are challenged, I am open and willing to provide clear explanations and rationales.": 4,
      "M_Influencing without Authority_I look for common ground and areas of agreement among peers or team members when making decisions.": 4,
      "M_Planning and Prioritizing_I plan my work effectively to meet timelines and quality.": 3,
      ...
      "M_Problem Solving_I look for ways to continuously improve things and learn from experiences.": 3,
      "M_Problem Solving_I use previous experiences to solve problems": 4,
      "M_Giving Feedback_I often talk to team members and guide them on their deliverables": 3,
      "M_Giving Feedback_I regularly tell team members what I think about their work and how they are doing and invite them to talk openly about their thoughts on the suggestions given.": 4,
      "M_Giving Feedback_I believe in open conversations and am open to receiving feedback from peers and team members": 4,
      "M_Entrepreneurial Mindset_I constantly look for new problems that need solutions - these can be at work or at a global level.": 3,
      "M_Entrepreneurial Mindset_I see the potential that problems bring with them.": 3
    },
    {
      ...
      "Influencing without Authority_I am open to listen to different views and opinions.": 5,
      "Influencing without Authority_When my thoughts and ideas are challenged, I am open and willing to provide clear explanations and rationales.": 4,
      "Influencing without Authority_I look for common ground and areas of agreement among peers or team members when making decisions.": 3,
      "Planning and Prioritizing_I plan my work effectively to meet timelines and quality.": 4,
      "Planning and Prioritizing_I prioritise my tasks and goals when there are multiple tasks at hand.": 3,
      "Planning and Prioritizing_I guide my team to plan and prioritise  work deliverables and meet timelines wihtout compromising on quality.": 5,
      "Problem Solving_I try to identify root causes while solving problems instead of finding quick fixes": 4,
      "Problem Solving_I use mine and the team's knowledge, reasoning and technical skills to evaluate key issues of a problem.": 5,
      "Problem Solving_I use best practices, gain new insights from problems  and processes to find innovative solutions.": 2,
      "Problem Solving_I look for ways to continuously improve things and learn from experiences.": 4,
      "Problem Solving_I use previous experiences to solve problems": 4,
      "Giving Feedback_I often talk to team members and guide them on their deliverables": 4,
      "Giving Feedback_I regularly tell team members what I think about their work and how they are doing and invite them to talk openly about their thoughts on the suggestions given.": 3,
      "Giving Feedback_I believe in open conversations and am open to receiving feedback from peers and team members": 4,
      "Entrepreneurial Mindset_I constantly look for new problems that need solutions - these can be at work or at a global level.": 2,
      "Entrepreneurial Mindset_I see the potential that problems bring with them.": 3,
       "M_Building Trust_I understand my team's needs and base my actions to support those needs.": 5,
      "M_Building Trust_My team members and I are approachable, openly communicate with each other and can share feedback without any fears.": 5,
      "M_Building Trust_I am always reliable / dependable": 5,
      ...
      "M_Influencing without Authority_When my thoughts and ideas are challenged, I am open and willing to provide clear explanations and rationales.": 5,
      "M_Influencing without Authority_I look for common ground and areas of agreement among peers or team members when making decisions.": 5,
      "M_Planning and Prioritizing_I plan my work effectively to meet timelines and quality.": 5,
      "M_Planning and Prioritizing_I prioritise my tasks and goals when there are multiple tasks at hand.": 4,
      "M_Planning and Prioritizing_I guide my team to plan and prioritise  work deliverables and meet timelines wihtout compromising on quality.": 4,
      "M_Problem Solving_I try to identify root causes while solving problems instead of finding quick fixes": 5,
      "M_Problem Solving_I use mine and the team's knowledge, reasoning and technical skills to evaluate key issues of a problem.": 5,
    ...
    },
    {
      ...
      "Giving Feedback_I regularly tell team members what I think about their work and how they are doing and invite them to talk openly about their thoughts on the suggestions given.": 4,
      "Giving Feedback_I believe in open conversations and am open to receiving feedback from peers and team members": 4,
      "Entrepreneurial Mindset_I constantly look for new problems that need solutions - these can be at work or at a global level.": 3,
      "Entrepreneurial Mindset_I see the potential that problems bring with them.": 4,
      "M_Building Trust_I understand my team's needs and base my actions to support those needs.": 3,
      "M_Building Trust_My team members and I are approachable, openly communicate with each other and can share feedback without any fears.": 4,
      "M_Building Trust_I am always reliable / dependable": 3,
      "M_Building Trust_I treat everyone fairly and equally.": 4,
      "M_Interpersonal Skills_I make others feel positive and respected  by my words and gestures.": 2,
      "M_Interpersonal Skills_I take initiative to connect with people.": 3,
      "M_Interpersonal Skills_I display enthusiasm while interacting with others.": 3,
      "M_Interpersonal Skills_I connect with people outside my team and often act as a liason between teams and stakeholder.": 3,
      "M_Delegation Skills_I effectively delegate tasks to team members based on their skills and expertise.": 4,
      "M_Delegation Skills_I provide clear instructions and set expectations when assigning tasks to team members": 4,
      "M_Delegation Skills_I monitor delegated tasks while allowing team members to approach their assigned responsibilities in their own way.": 3,
      "M_Assertiveness_I am confident and clear while interacting with peers, reports, managers and stakeholders.": 4,
      ...
      "M_Assertiveness_I come across as assertive when I interact with my team, managers and higherups.": 2,
      "M_Influencing without Authority_I use logic, facts and data to influence decisions and actions among peers and team members.": 4,
      "M_Influencing without Authority_I am open to listen to different views and opinions.": 2,
      "M_Influencing without Authority_When my thoughts and ideas are challenged, I am open and willing to provide clear explanations and rationales.": 4,
      "M_Influencing without Authority_I look for common ground and areas of agreement among peers or team members when making decisions.": 3,
      "M_Planning and Prioritizing_I plan my work effectively to meet timelines and quality.": 4,
      "M_Planning and Prioritizing_I prioritise my tasks and goals when there are multiple tasks at hand.": 4,
      "M_Planning and Prioritizing_I guide my team to plan and prioritise  work deliverables and meet timelines wihtout compromising on quality.": 4,
      "M_Problem Solving_I try to identify root causes while solving problems instead of finding quick fixes": 4,
      "M_Problem Solving_I use mine and the team's knowledge, reasoning and technical skills to evaluate key issues of a problem.": 4,
      "M_Problem Solving_I use best practices, gain new insights from problems  and processes to find innovative solutions.": 3,
      "M_Problem Solving_I look for ways to continuously improve things and learn from experiences.": 2,
      "M_Problem Solving_I use previous experiences to solve problems": 5,
      "M_Giving Feedback_I often talk to team members and guide them on their deliverables": 4,
      "M_Giving Feedback_I regularly tell team members what I think about their work and how they are doing and invite them to talk openly about their thoughts on the suggestions given.": 2,
      "M_Giving Feedback_I believe in open conversations and am open to receiving feedback from peers and team members": 2,
      "M_Entrepreneurial Mindset_I constantly look for new problems that need solutions - these can be at work or at a global level.": 2,
      "M_Entrepreneurial Mindset_I see the potential that problems bring with them.": 4
    },
    {
      "Name": "Pritam Hegishte",
      ...
      "Entrepreneurial Mindset_I see the potential that problems bring with them.": 5,
      "M_Building Trust_I understand my team's needs and base my actions to support those needs.": 4,
      "M_Building Trust_My team members and I are approachable, openly communicate with each other and can share feedback without any fears.": 2,
      "M_Building Trust_I am always reliable / dependable": 4,
      "M_Building Trust_I treat everyone fairly and equally.": 4,
      "M_Interpersonal Skills_I make others feel positive and respected  by my words and gestures.": 4,
      "M_Interpersonal Skills_I take initiative to connect with people.": 4,
      ...
      "M_Assertiveness_I am confident and clear while interacting with peers, reports, managers and stakeholders.": 4,
      "M_Assertiveness_I am sure of my perspectives - issues, solutions, way forward etc., and state them clearly to others.": 4,
      "M_Assertiveness_I can challenge the views of others when required.": 3,
      "M_Assertiveness_I come across as assertive when I interact with my team, managers and higherups.": 4,
      "M_Influencing without Authority_I use logic, facts and data to influence decisions and actions among peers and team members.": 3,
      "M_Influencing without Authority_I am open to listen to different views and opinions.": 3,
      "M_Influencing without Authority_When my thoughts and ideas are challenged, I am open and willing to provide clear explanations and rationales.": 3,
      "M_Influencing without Authority_I look for common ground and areas of agreement among peers or team members when making decisions.": 4,
      "M_Planning and Prioritizing_I plan my work effectively to meet timelines and quality.": 3,
      "M_Planning and Prioritizing_I prioritise my tasks and goals when there are multiple tasks at hand.": 2,
      "M_Planning and Prioritizing_I guide my team to plan and prioritise  work deliverables and meet timelines wihtout compromising on quality.": 3,
      "M_Problem Solving_I try to identify root causes while solving problems instead of finding quick fixes": 3,
      "M_Problem Solving_I use mine and the team's knowledge, reasoning and technical skills to evaluate key issues of a problem.": 4,
      "M_Problem Solving_I use best practices, gain new insights from problems  and processes to find innovative solutions.": 4,
      "M_Problem Solving_I look for ways to continuously improve things and learn from experiences.": 4,
      "M_Problem Solving_I use previous experiences to solve problems": 4,
      "M_Giving Feedback_I often talk to team members and guide them on their deliverables": 4,
      "M_Giving Feedback_I regularly tell team members what I think about their work and how they are doing and invite them to talk openly about their thoughts on the suggestions given.": 3,
      "M_Giving Feedback_I believe in open conversations and am open to receiving feedback from peers and team members": 1,
      "M_Entrepreneurial Mindset_I constantly look for new problems that need solutions - these can be at work or at a global level.": 3,
      "M_Entrepreneurial Mindset_I see the potential that problems bring with them.": 3
    },
    {
    ...
    {
      "Name": "Rahul Durgvijay Singh",
      "M_Building Trust_I understand my team's needs and base my actions to support those needs.": 3,
      "M_Building Trust_My team members and I are approachable, openly communicate with each other and can share feedback without any fears.": 3,
      "M_Building Trust_I am always reliable / dependable": 3,
      "M_Building Trust_I treat everyone fairly and equally.": 3,
      "M_Interpersonal Skills_I make others feel positive and respected  by my words and gestures.": 3,
      "M_Interpersonal Skills_I take initiative to connect with people.": 4,
      "M_Interpersonal Skills_I display enthusiasm while interacting with others.": 3,
      "M_Interpersonal Skills_I connect with people outside my team and often act as a liason between teams and stakeholder.": 3,
      "M_Delegation Skills_I effectively delegate tasks to team members based on their skills and expertise.": 4,
      ...
      "M_Planning and Prioritizing_I guide my team to plan and prioritise  work deliverables and meet timelines wihtout compromising on quality.": 3,
      "M_Problem Solving_I try to identify root causes while solving problems instead of finding quick fixes": 3,
      "M_Problem Solving_I use mine and the team's knowledge, reasoning and technical skills to evaluate key issues of a problem.": 3,
      "M_Problem Solving_I use best practices, gain new insights from problems  and processes to find innovative solutions.": 3,
      "M_Problem Solving_I look for ways to continuously improve things and learn from experiences.": 3,
      "M_Problem Solving_I use previous experiences to solve problems": 3,
      "M_Giving Feedback_I often talk to team members and guide them on their deliverables": 3,
      "M_Giving Feedback_I regularly tell team members what I think about their work and how they are doing and invite them to talk openly about their thoughts on the suggestions given.": 3,
      "M_Giving Feedback_I believe in open conversations and am open to receiving feedback from peers and team members": 3,
      "M_Entrepreneurial Mindset_I constantly look for new problems that need solutions - these can be at work or at a global level.": 3,
      "M_Entrepreneurial Mindset_I see the potential that problems bring with them.": 3
    },
    {
      ...
      "Interpersonal Skills_I take initiative to connect with people.": 5,
      "Interpersonal Skills_I display enthusiasm while interacting with others.": 4,
      "Interpersonal Skills_I connect with people outside my team and often act as a liason between teams and stakeholder.": 3,
      "Delegation Skills_I effectively delegate tasks to team members based on their skills and expertise.": 5,
      "Delegation Skills_I provide clear instructions and set expectations when assigning tasks to team members": 5,
      "Delegation Skills_I monitor delegated tasks while allowing team members to approach their assigned responsibilities in their own way.": 5,
      "Assertiveness_I am confident and clear while interacting with peers, reports, managers and stakeholders.": 5,
    ...
    },
    {
      ...
      "Influencing without Authority_When my thoughts and ideas are challenged, I am open and willing to provide clear explanations and rationales.": 5,
      "Influencing without Authority_I look for common ground and areas of agreement among peers or team members when making decisions.": 5,
      "Planning and Prioritizing_I plan my work effectively to meet timelines and quality.": 0,
      "Planning and Prioritizing_I prioritise my tasks and goals when there are multiple tasks at hand.": 3,
      "Planning and Prioritizing_I guide my team to plan and prioritise  work deliverables and meet timelines wihtout compromising on quality.": 3,
      "Problem Solving_I try to identify root causes while solving problems instead of finding quick fixes": 4,
      "Problem Solving_I use mine and the team's knowledge, reasoning and technical skills to evaluate key issues of a problem.": 5,
      ...
      "Problem Solving_I look for ways to continuously improve things and learn from experiences.": 4,
      "Problem Solving_I use previous experiences to solve problems": 4,
      "Giving Feedback_I often talk to team members and guide them on their deliverables": 3,
      "Giving Feedback_I regularly tell team members what I think about their work and how they are doing and invite them to talk openly about their thoughts on the suggestions given.": 3,
      "Giving Feedback_I believe in open conversations and am open to receiving feedback from peers and team members": 3,
      "Entrepreneurial Mindset_I constantly look for new problems that need solutions - these can be at work or at a global level.": 2,
      "Entrepreneurial Mindset_I see the potential that problems bring with them.": 2,
      "M_Building Trust_I understand my team's needs and base my actions to support those needs.": 5,
      "M_Building Trust_My team members and I are approachable, openly communicate with each other and can share feedback without any fears.": 5,
      "M_Building Trust_I am always reliable / dependable": 5,
      "M_Building Trust_I treat everyone fairly and equally.": 5,
    ...
    },
    {
      ...
      "M_Interpersonal Skills_I take initiative to connect with people.": 3,
      "M_Interpersonal Skills_I display enthusiasm while interacting with others.": 3,
      "M_Interpersonal Skills_I connect with people outside my team and often act as a liason between teams and stakeholder.": 2,
      "M_Delegation Skills_I effectively delegate tasks to team members based on their skills and expertise.": 4,
      "M_Delegation Skills_I provide clear instructions and set expectations when assigning tasks to team members": 4,
      "M_Delegation Skills_I monitor delegated tasks while allowing team members to approach their assigned responsibilities in their own way.": 3,
      "M_Assertiveness_I am confident and clear while interacting with peers, reports, managers and stakeholders.": 3,
      ...
      "M_Influencing without Authority_I use logic, facts and data to influence decisions and actions among peers and team members.": 3,
      "M_Influencing without Authority_I am open to listen to different views and opinions.": 2,
      "M_Influencing without Authority_When my thoughts and ideas are challenged, I am open and willing to provide clear explanations and rationales.": 2,
      "M_Influencing without Authority_I look for common ground and areas of agreement among peers or team members when making decisions.": 3,
      "M_Planning and Prioritizing_I plan my work effectively to meet timelines and quality.": 4,
      "M_Planning and Prioritizing_I prioritise my tasks and goals when there are multiple tasks at hand.": 3,
      "M_Planning and Prioritizing_I guide my team to plan and prioritise  work deliverables and meet timelines wihtout compromising on quality.": 3,
      "M_Problem Solving_I try to identify root causes while solving problems instead of finding quick fixes": 4,
      "M_Problem Solving_I use mine and the team's knowledge, reasoning and technical skills to evaluate key issues of a problem.": 4,
      "M_Problem Solving_I use best practices, gain new insights from problems  and processes to find innovative solutions.": 2,
      "M_Problem Solving_I look for ways to continuously improve things and learn from experiences.": 4,
      "M_Problem Solving_I use previous experiences to solve problems": 4,
      "M_Giving Feedback_I often talk to team members and guide them on their deliverables": 4,
      "M_Giving Feedback_I regularly tell team members what I think about their work and how they are doing and invite them to talk openly about their thoughts on the suggestions given.": 3,
      "M_Giving Feedback_I believe in open conversations and am open to receiving feedback from peers and team members": 3,
      "M_Entrepreneurial Mindset_I constantly look for new problems that need solutions - these can be at work or at a global level.": 2,
      "M_Entrepreneurial Mindset_I see the potential that problems bring with them.": 2
    },
    {
      "Name": "Sagar Satish Nawathe",
      "Building Trust_I understand my team's needs and base my actions to support those needs.": 5,
    ...
    },
    {
      ...
      "M_Building Trust_I treat everyone fairly and equally.": 3,
      "M_Interpersonal Skills_I make others feel positive and respected  by my words and gestures.": 2,
      "M_Interpersonal Skills_I take initiative to connect with people.": 1,
      "M_Interpersonal Skills_I display enthusiasm while interacting with others.": 1,
      "M_Interpersonal Skills_I connect with people outside my team and often act as a liason between teams and stakeholder.": 1,
      "M_Delegation Skills_I effectively delegate tasks to team members based on their skills and expertise.": 3,
      "M_Delegation Skills_I provide clear instructions and set expectations when assigning tasks to team members": 4,
      "M_Delegation Skills_I monitor delegated tasks while allowing team members to approach their assigned responsibilities in their own way.": 2,
      "M_Assertiveness_I am confident and clear while interacting with peers, reports, managers and stakeholders.": 4,
      "M_Assertiveness_I am sure of my perspectives - issues, solutions, way forward etc., and state them clearly to others.": 4,
      "M_Assertiveness_I can challenge the views of others when required.": 3,
      "M_Assertiveness_I come across as assertive when I interact with my team, managers and higherups.": 3,
      "M_Influencing without Authority_I use logic, facts and data to influence decisions and actions among peers and team members.":2,
      "M_Influencing without Authority_I am open to listen to different views and opinions.": 4,
      "M_Influencing without Authority_When my thoughts and ideas are challenged, I am open and willing to provide clear explanations and rationales.": 4,
      "M_Influencing without Authority_I look for common ground and areas of agreement among peers or team members when making decisions.": 3,
      "M_Planning and Prioritizing_I plan my work effectively to meet timelines and quality.": 3,
    ...
    },
    {
      ...
      "Assertiveness_I can challenge the views of others when required.": 3,
      "Assertiveness_I come across as assertive when I interact with my team, managers and higherups.": 3,
      "Influencing without Authority_I use logic, facts and data to influence decisions and actions among peers and team members.": 3,
      "Influencing without Authority_I am open to listen to different views and opinions.": 4,
      "Influencing without Authority_When my thoughts and ideas are challenged, I am open and willing to provide clear explanations and rationales.": 4,
      "Influencing without Authority_I look for common ground and areas of agreement among peers or team members when making decisions.": 4,
      "Planning and Prioritizing_I plan my work effectively to meet timelines and quality.": 4,
      "Planning and Prioritizing_I prioritise my tasks and goals when there are multiple tasks at hand.": 5,
      "Planning and Prioritizing_I guide my team to plan and prioritise  work deliverables and meet timelines wihtout compromising on quality.": 5,
      "Problem Solving_I try to identify root causes while solving problems instead of finding quick fixes": 3,
      "Problem Solving_I use mine and the team's knowledge, reasoning and technical skills to evaluate key issues of a problem.": 4,
      "Problem Solving_I use best practices, gain new insights from problems  and processes to find innovative solutions.": 5,
      "Problem Solving_I look for ways to continuously improve things and learn from experiences.": 4,
      "Problem Solving_I use previous experiences to solve problems": 5,
    ...
    },
    {
      ...
      "Building Trust_I am always reliable / dependable": 5,
      "Building Trust_I treat everyone fairly and equally.": 4,
      "Interpersonal Skills_I make others feel positive and respected  by my words and gestures.": 3,
      "Interpersonal Skills_I take initiative to connect with people.": 5,
      "Interpersonal Skills_I display enthusiasm while interacting with others.": 4,
      "Interpersonal Skills_I connect with people outside my team and often act as a liason between teams and stakeholder.": 4,
      "Delegation Skills_I effectively delegate tasks to team members based on their skills and expertise.": 3,
      ...
      "Assertiveness_I come across as assertive when I interact with my team, managers and higherups.": 5,
      "Influencing without Authority_I use logic, facts and data to influence decisions and actions among peers and team members.": 4,
      "Influencing without Authority_I am open to listen to different views and opinions.": 3,
      "Influencing without Authority_When my thoughts and ideas are challenged, I am open and willing to provide clear explanations and rationales.": 4,
      "Influencing without Authority_I look for common ground and areas of agreement among peers or team members when making decisions.": 4,
      "Planning and Prioritizing_I plan my work effectively to meet timelines and quality.": 5,
      "Planning and Prioritizing_I prioritise my tasks and goals when there are multiple tasks at hand.": 5,
      ...
      "Problem Solving_I try to identify root causes while solving problems instead of finding quick fixes": 5,
      "Problem Solving_I use mine and the team's knowledge, reasoning and technical skills to evaluate key issues of a problem.": 4,
      "Problem Solving_I use best practices, gain new insights from problems  and processes to find innovative solutions.": 3,
      "Problem Solving_I look for ways to continuously improve things and learn from experiences.": 4,
      "Problem Solving_I use previous experiences to solve problems": 5,
      "Giving Feedback_I often talk to team members and guide them on their deliverables": 5,
      "Giving Feedback_I regularly tell team members what I think about their work and how they are doing and invite them to talk openly about their thoughts on the suggestions given.": 4,
      ...
      "M_Entrepreneurial Mindset_I see the potential that problems bring with them.": 4
        ...
        # print(results)
        base64_images_list = generate_individual_graphs(results, bar_width=0.3)

        for result in results:
            try:
                score_analysis_data=None
                for score in score_analysis_datas:
                    if score["Name"] == result["name"]:
                        score_analysis_data=score
                # print(result)
                print(score_analysis_data["Name"])
                print(result["name"])
                result
                t_data = transform_data(result)
                with open(
                    "graphsAndReports/report_page_after_contents.png", "rb"
                ) as image_file:
                    encoded_string = base64.b64encode(image_file.read()).decode("utf-8")
                with open("graphsAndReports/med_logo.jpg", "rb") as image_file:
                    med_logo_encoded_string = base64.b64encode(
                        image_file.read()
                    ).decode("utf-8")
                with open("graphsAndReports/meeraq_logo.png", "rb") as image_file:
                    meeraq_logo_encoded_string = base64.b64encode(
                        image_file.read()
                    ).decode("utf-8")
                data_for_score_analysis = format_data_for_html(score_analysis_data)
                # print(data_for_score_analysis)
                frequency_analysis_data=freq_data_get(data_for_score_analysis)
                content = {
                    "name": result["name"],
                    "data_for_score_analysis": data_for_score_analysis,
                    "data": t_data,
                    "frequency_analysis_data": frequency_analysis_data,
                    "image_base64": base64_images_list[result["name"]],
                    "page_after_content_image_base64": encoded_string,
                    "med_logo_encoded_string": med_logo_encoded_string,
                    "meeraq_logo_encoded_string": meeraq_logo_encoded_string,
                }

                email_message = render_to_string(
                    "assessment/report/medline_report.html", content
                )
                pdfkit_options = {
                ...
                name = result["name"]
                pdf_path = f"graphsAndReports/reportss/{name} Report.pdf"
                ...
                # response[
                #     "Content-Disposition"
                # ] = f'attachment; filename={f"{name} Report.pdf"}'
                # return response
