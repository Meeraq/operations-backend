...
def session_view(request):
    ...
    user_data = get_user_data(user)
    if user_data:
        response = Response({"isAuthenticated": True, "user": user_data})
        response["X-CSRFToken"] = get_token(request)
        return response
    else:
        return Response({"error": "Invalid user type"}, status=400)
...
def receive_coach_consent(request):
    ...
    for coach_status in project.coaches_status.all():
        print("inside loop", coach_status.coach.id == request.data.get("coach_id", ""))
        try:
            print("tryu")
            if coach_status.coach.id == int(request.data.get("coach_id", "")):
                print("inside ig ...")
    ...
    print("outside loop", request.data["status"])
